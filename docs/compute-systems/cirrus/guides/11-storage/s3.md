...
In addition to private S3 buckets CIRRUS also has a bucket that is published externally with anonymous read only access via https. 
...
Once you have access to CIRRUS S3 with credentials you can publish data to be externally available. This could be useful for sharing objects with others or accesing internal data for github hosted runners. This is similar to a public ftp server that some may be familiar with.

### Add objects

The bucket is named s3://public. You can write to this bucket with your credentials and should use a user/project idetifying prefix for objects. Don't upload objects as s3://public/my-object.tar, use a format like s3://public/username/my-object.tar. 

### Access objects over https

The URL is https://pub.k8s.ucar.edu and going to this url will give a not found error. The aws cli and s5cmd do not work with this url, it is a http representation of a bucket. By default we are not publicly listing objects and to access an object you need to know its name. From the add objects example the object would be available as https://pub.k8s.ucar.edu/username/my-object.tar. 