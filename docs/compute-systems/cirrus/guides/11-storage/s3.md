...
When using either `aws` or `s5cmd` you need to use 2 additional flags --endpoint-url=https://s3.k8s.ucar.edu:5443 and --no-verify-ssl. We are working to remove the need for the --no-verify-ssl. Currently the connection is secure over SSL but the cli utilites do not recognize the signed certificate.

__s5cmd__

[s5cmd](https://github.com/peak/s5cmd)

s5cmd is a community project for interacting with s3 resources. It uses the same configuration files above. Generally it is much faster the teh `aws` cli or `s3cmd` project.
