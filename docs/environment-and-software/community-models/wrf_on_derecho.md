# How to compile and run WRF on Derecho
...
!!! tip "Pre-compiled WRF on Derecho"
...
### Load Balancing and Optimizing WRF on Derecho

WRF runtime is configured predominantly via a specified `namelist.input` file that details important variables and other configurations for the model simulation. 
The file `README.namelist` details all the variables that may be modified to your needs, with the latest version detailed at the [WRF Github for `README.namelist`](https://github.com/wrf-model/WRF/blob/master/run/README.namelist).

To note, in order to achieve optimal performance on Derecho, some primary considerations need to be made.

#### Determining node and MPI process counts with `wrf-procs-finder`

Specific to Derecho, a convenience tool `wrf-procs-finder` is provided through the `ncarenv` environment.
This tool determines the minimum and maximum number of processors and nodes based on number grid points.
The script requires the number of CPU cores per node on the system (default 128 on Derecho)
and the domain size via the namelist variables `e_we` and `e-sn` or the `namelist.input` file itself.
Using this information, the script will provide a range of MPI process count per node suggestions 
where given the configuration at which the simulation will likely run optimally. 

See below for an example of how to use the tool or reach out to the [Research Computing Help Desk](https://rchelp.ucar.edu) for specific support with configuring WRF on Derecho.

```bash
# Print usage and help text for the script
wrf-procs-finder -h

# Example usage
wrf-procs-finder --e_we 1000 --e_sn 1000
# OR
wrf-procs-finder --namelist /path/to/namelist
...
------------------------------------------------------------
  e_we = 1000
  e_sn = 1000
  Total grid points: 1000 x 1000 = 1,000,000

  nprocs range:
    Minimum.......................    100 procs  =>  1 node @ 100 procs/node
    Maximum.......................   6528 procs  =>  51 nodes @ 128 procs/node

  Decomposition for 6528 procs:
      Layout    :   68 x 96  tiles
      Avg tile  : ~ 14 x 10  grid points
```

#### NetCDF and Parallel NetCDF

Effective usage of the netCDF and parallel netcdf libraries are important for I/O performanceon the scratch system. 
Serial or parallel NetCDF options can be specified in the run-time WRF namelist, and the parallel netCDF I/O is often significantly faster than the serial I/O option. 
Therefore, we recommend to build WRF with the `netcdf` and `parallel-netcdf` module libraries. This is necessary only when compiling WRF from source.
With these two modules, we set the following environmental variables before compiling WRF
...
# Recommended environment variables for WRF use cases
export NETCDF_classic=1                # Set classic (CDF1) as default
export WRFIO_NCD_LARGE_FILE_SUPPORT=1  # Enable 64-bit offset format (CDF2) of netcdf files
export USE_NETCDF4_FEATURES=0.         # NetCDF4 compression (serial) with the HDF5 module can be slow
```

#### Community Open Source Tools for WRF
...
### Example WRF job scripts and workflow
...
To know more about starting and managing jobs, please refer to the [documentation](../../../pbs).

## Running Long Simulations with Restart Capability
For simulations that exceed the [wall-clock time limits of Derecho](../../../pbs/charging) or for operational workflows that require regular checkpoints, WRF supports restart capability, allowing you to break up long simulations into multiple shorter runs without losing progress.
