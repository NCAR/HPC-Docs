...
If you want to compile WRF-Chem, continue with the following steps. Otherwise, follow the instructions in the NCAR HPC Documentation for [compiling atmospheric WRF and WPS on Derecho](./wrf_on_derecho.md).
...
Before downloading the codes, review the default [quotas and purging policies](../../storage-systems/glade/index.md) of different GLADE file spaces. This will help you determine the most appropriate location to store the source codes.
...
Please refer to the instructions to [compile WPS on Derecho](./wrf_on_derecho.md#compiling-the-wrf-preprocessing-system-wps), if you prefer to do so.

To compile WRF using the INTEL (ifx/icx) LLVM compiler (recommended), it is advised to compile within an [interactive job queue](../../pbs/index.md#qinteractive-and-execcasper) to avoid issues with required memory during compilation. To accomplish that use the following command on Derecho:
```bash
...
On Derecho, this is done by loading pre-loaded modules. Please refer to [Modules](../user-environment/modules.md) for more information. The modules and environment variables are needed regardless of using the Cmake build or by traditional method.
...
qsub script_name
```
To know more about starting and managing jobs, please refer to the [documentation](../../pbs/index.md).
...
For simulations that exceed the [wall-clock time limits of Derecho](../../pbs/charging.md) or for operational workflows that require regular checkpoints, WRF supports restart capability, allowing you to break up long simulations into multiple shorter runs without losing progress.
