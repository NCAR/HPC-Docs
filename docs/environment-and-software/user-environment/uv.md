...
To read more about `uv`, visit the [official documentation](https://docs.astral.sh/uv/).


## Using `uv` on NCAR Systems

On NCAR systems, we provide a `uv` module that:

- provides access to the uv command.
- allows you to create lightweight, fast Python environments for your
projects.
- supports modern Python packaging standards including `pyproject.toml`
and lock files for reproducibility.
...
Before using `uv` on NCAR systems, keep in mind:
...
your home directory quota. Virtual environments can be several hundred
megabytes or larger depending on installed packages.
