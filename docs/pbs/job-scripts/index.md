...
2.  **Loading and reporting the specific module environment required for this job.**
    ...
    While strictly not necessary (in general, the default modules will be loaded anyway), we recommend this as best practice as it facilitates debugging and reproducing later.  (While the system default modules will change over time, manually specifying module versions allows you to recreate the same execution environment in the future.)
...
specifics of the job to depend slightly on the PBS execution
