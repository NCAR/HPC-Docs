...
!!! note "Introducing Hierarchical Storage Management (HSM) for *infrequently read* data sets"
    Campaign Storage is logically connected to the [Quasar](../quasar/index.md) tape library, allowing users to easily request old - but still important - data to be moved to "cold storage." This feature leverages the hierarchical storage management (HSM) capabilities [built into](https://www.ibm.com/support/pages/system/files/inline-files/Spectrum_Archive_Solution_v6.pdf) Spectrum Scale and Spectrum Archive.  HSM is ideal for long-term storage of data sets that are rarely read but are being kept perhaps for a publication requirement, or as "just-in-case" backups within a computational campaign.  Once HSM migrates data to tape, disk space - and project quota - are reclaimed for more productive, active use.
